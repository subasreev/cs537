...
readi(struct inode *ip, char *dst, uint off, uint n)
{
	...
	//p5b support for small files
	if(ip->type == T_SMALLFILE){
		//cprintf("reading small file off=%d n=%d\n", off, n);
		//read data packed inside the inode
		if(off > ip->size || off + n < off) //inode addrs range from 0-51 
			return -1;
		if(off + n > ip->size)
			n = ip->size - off;
		//ip addrs start from 28
		//cprintf("data in read=%s\n",((char *)ip)+28);
		memmove(dst, ((char *)ip)+28+off, n);
	}
	else{
...
writei(struct inode *ip, char *src, uint off, uint n)
{
	...
	//p5b support for smallfile
	if(ip->type == T_SMALLFILE){
		if(off > ip->size || off + n < off)
			return -1;
		if(off + n > 52)
			n = 52 - off;
		char *buf = ((char *)ip)+28+off;
		int i;
		for(i=0;i<n;i++){
			buf[i] = src[i];
			//cprintf("%x %x %x\n",buf, buf+i,((char *)ip)+28+i);
		}
		//cprintf("in write filetype=%d\n",ip->type);
		//cprintf("%s\n",((char *)ip)+28);

		ip->size = off+n; 
		//cprintf("size of small file=%d\n",ip->size);
		iupdate(ip);
	}
	else{
		...
		if(n > 0 && off > ip->size){
			ip->size = off;
			//cprintf("size of regular file=%d\n",ip->size);
			iupdate(ip);
		}
	}
	return n;
